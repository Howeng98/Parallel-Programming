...
// for right
bool exchange_right(int neighbor_total, int total) {
  // early terminate
  if (temp_buffer[neighbor_total-1] <= data[0]) return true;
...
// for left
bool exchange_left(int neighbor_total, int total) {
  if (temp_buffer[0] >= data[total-1]) return true;
...
int main(int argc, char** argv) {
  ...
  while (!result) {
    ...
    if (flag) {
      MPI_Allreduce(&now, &result, 1, MPI_C_BOOL, MPI_LAND, mpi_comm);
    }
    flag = (flag == 0) ? 1 : 0;
  }
