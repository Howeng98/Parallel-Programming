...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
  ...
  adjustX = FLAG_X;
  adjustY = FLAG_Y;
  __shared__ unsigned char R_arr[5][263];
  __shared__ unsigned char G_arr[5][263];
  __shared__ unsigned char B_arr[5][263];

  for (v = -yBound; v < yBound + adjustY; v++) {
    for (u = -xBound; u < xBound + adjustX; u++) {
      if ((x + u) >= 0 && (x + u) < width && y + v >= 0 && y + v < height) {
        R_arr[v + yBound][threadIdx.x + u + 2] = s[channels * (width * (y + v) + (x + u)) + 2];
        G_arr[v + yBound][threadIdx.x + u + 2] = s[channels * (width * (y + v) + (x + u)) + 1];
        B_arr[v + yBound][threadIdx.x + u + 2] = s[channels * (width * (y + v) + (x + u)) + 0];
      }
    }
  }
  __syncthreads();
...
int main(int argc, char **argv) {
  ...
  unsigned char *host_t;
  unsigned char *d_src;
  unsigned char *d_dst;

  int threads = 256;
  int x = (width % 256) ? width / 256 + 1 : width / 256;
  int y = height;
  dim3 blocks(x, y);
  ...
  sobel<<<blocks, threads>>>(d_src, host_t, height, width, channels);
