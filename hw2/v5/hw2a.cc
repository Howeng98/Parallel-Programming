...
inline void* threadpool_thread(void* threadpool) {
  ...
  while (true) {
    ...
    while (record_now <= pool->width) {
      ...
      if (length_square_see[0] >= constraint || repeats[0] >= pool->iters) {
        ...
        record_now += 1;
      } if (length_square_see[1] >= constraint || repeats[1] >= pool->iters) {
    ...
    if (!done[0]) {
      ...
      while (repeats[0] < pool->iters && length_square < constraint) {
        tmp = x * x - y * y + x0;
        y = 2 * x * y + y0;
        x = tmp;
      ...
      while (repeats[1] < pool->iters && length_square < constraint) {
        tmp = x * x - y * y + x0;
        y = 2 * x * y + y0;
        x = tmp;
        length_square = x * x + y * y;
