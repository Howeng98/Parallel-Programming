...
int main(int argc, char** argv) {
  ...
  check_point = MPI_Wtime();
  #pragma omp parallel num_threads(ncpus) shared(i)
  {
    // mandelbrot set
    bool done[2];
    int repeats[2];
  ...
  cpu_time += (MPI_Wtime() - check_point);

  if (rank == 0) {
    check_point = MPI_Wtime();
    MPI_Reduce(MPI_IN_PLACE, image, width * height, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    comm_time += (MPI_Wtime() - check_point);

    check_point = MPI_Wtime();
    write_png(filename, iters, width, height, image);
    io_time += (MPI_Wtime() - check_point);
    cout << cpu_time << endl;
    cout << comm_time << endl;
    cout << io_time << endl;
  } else {
