...
for thread in tqdm(thread_num):
  cmd = f"srun -c{thread} -n4 ./hw2b experiment.png {testcase}"
...
labels = [str(i*4) for i in thread_num]
...
fig, ax = plt.subplots()
total_time = [result_cpu[i]+result_io[i]+result_comm[i] for i in range(len(thread_num))]
